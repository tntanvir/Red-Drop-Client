...
export function StickyNavbar() {
  ...
  const navList = (
    <ul className="mb-4 mt-2 flex flex-col gap-2 lg:mb-0 lg:mt-0 lg:flex-row lg:items-center lg:gap-6">
      ...
      >
        <Link to="/resivedPost" className="flex items-center">
          Post Feed
        </Link>
      </Typography>
      <Typography
  ...
  useEffect(() => {
    if (sessionStorage.getItem('token')) {
      fetch(`https://red-drop-django.vercel.app/authore/user/${sessionStorage.getItem('username')}/`)
        .then(res => res.json())
  ...
  return (
    <>
      <Navbar className={`sticky top-0 z-50 h-max max-w-full rounded-none py-1 px-4 lg:px-8 lg:py-1 bg-opacity-100 border-none  blur-none bg-yellow-700 text-white ${nav ? 'shadow-md' : 'shadow-none'}`}>
        <div className="flex items-center justify-between text-blue-gray-900">
          ...
          <div className="flex items-center gap-4">
            ...
            {user?.user?.email ?
              <Menu>
                <MenuHandler>
                  {user && <Avatar className="cursor-pointer" src={user?.image} size="sm" />}
                </MenuHandler>
                <MenuList>
                  <Link to={"/account/deshboard"}>
                    <MenuItem>
                      Account
                    </MenuItem>
                  </Link>
      ...
      >
        <Card className="mx-auto w-full max-w-[24rem]">
          ...
          <CardBody className="flex flex-col gap-4">
            ...
            <Button disabled variant="gradient" onClick={handleOpen} fullWidth>
              Sign In
            </Button>
